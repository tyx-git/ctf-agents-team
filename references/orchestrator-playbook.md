...
- **比赛级文件唯一写者/读者**：`limit.md`、`task_plan.md`、`findings.md`、`progress.md`、`flag.log` 只允许 Lead Agent 读写；子 Agent 只接收 Lead 注入的必要摘要
...
| image/audio/archive/Forensics 提取出的编码文本/图片/压缩包 | misc-agent | 出现 native 逻辑或 web 层 | pyjails, bashjails, encodings, games-and-vms, rf-sdr, dns |
...
| URL/source audit/auth/injection/SSTI | web-agent | 原始 PCAP/pcapng → forensics；Forensics 提取出的 blob/编码层 → misc | — |
...
| Docker/container escape/CVE逃逸/Capability | knowledge/docker-escape.md |
| Kernel pwn/LKM/UAF/tty_struct/BPF JIT | knowledge/kernel-pwn.md |
...
### 最终 WP 写作时机
- 单题模式：Lead Agent 端到端确认并达到 `verified` 后
- Solo 模式：子 Agent 本地确认 flag 并达到 `solved` 后立即写题目 WP，Lead Agent 汇总后再标记 `verified`
- 格式严格遵循 references/wp-format.md

### flag.log 维护（Lead Agent 唯一写者）

**核心原则**：仅 Lead Agent 有权写入比赛根目录的 `flag.log`。子 Agent 不直接写入，只产出题目目录下的 `flag.found`。

Solo 模式汇总时直接调用脚本；模型无需读取脚本内容：
...
只需要使用脚本 JSON 输出：
- `summary.appended` — 新增写入数量
- `summary.skipped` — 已存在而跳过数量
- `summary.invalid` — 格式无效数量
- `appended[]` / `skipped[]` / `invalid[]` — 需要记录到 `progress.md` 的明细
...
3. 对每条记录使用 `AddExp.py --commit` 追加（自动去重 + 同步全部仓库）：
   ...
   ```
   `AddExp.py` 自动发现 `~/.claude/skills/ctf-agents-team/exp/` 和 `~/.codex/skills/ctf-agents-team/exp/` 并同时写入，确保两个用户级仓库一致。**不写入项目本地 `$SKILL_DIR/exp/`**（如需将用户级最新内容拉到项目本地，使用 `python3 "$SKILL_DIR/scripts/AddExp.py" --debug-syn` 手动同步）。
4. 合并完成后执行 `python3 "$SKILL_DIR/scripts/ClearExp.py" $COMPETITION_DIR` 清理所有 `exp_candidate.jsonl`
...
- **比赛限制**: [Lead Agent 从 limit.md 读取后填入；空则写“无特殊限制”]
- **题目列表** (按优先级排序，最多 5 道):
  1. [题目名A] — [目录路径]
  2. [题目名B] — [目录路径]
  ...
...
## 你可以加载的技术参考
先由 Lead Agent 提供 `$SKILL_DIR`，再按显式映射读取 specialist reference：

| 品类 | reference |
|------|-----------|
| `re` | `$SKILL_DIR/references/reverse-agent.md` |
| `web` | `$SKILL_DIR/references/web-agent.md` |
| `pwn` | `$SKILL_DIR/references/pwn-agent.md` |
| `mobile` | `$SKILL_DIR/references/mobile-agent.md` |
| `misc` | `$SKILL_DIR/references/misc-agent.md` |
| `crypto` | `$SKILL_DIR/references/crypto-agent.md` |
| `forensics` | `$SKILL_DIR/references/forensics-agent.md` |

- 按需读取 `$SKILL_DIR/knowledge/` 下的深度文档（参见 orchestrator-playbook.md 知识加载指南）

## 经验库
- 读取 `$SKILL_DIR/exp/[品类]/[品类].jsonl` 查看历史经验（如 `$SKILL_DIR/exp/pwn/pwn.jsonl`）
  - **Mobile Agent 注意**：Mobile 经验存储在 `$SKILL_DIR/exp/re/re.jsonl`（通过 `challenge: "Mobile"` 字段区分），请读取该文件并在搜索时用 `grep '"challenge": "Mobile"'` 过滤 Mobile 相关条目。
- 解完每道题后，若有可复用经验则写入题目目录下 `exp_candidate.jsonl`（纯模板题/签到题跳过）
- **注意**：经验库路径是 `$SKILL_DIR/exp/`，不是比赛目录；写入经验候选到题目目录，由 Lead Agent 最终合并
...
5. **验证** — 本地确认/复现 flag，状态最多达到 `solved`
6. **交付（立即执行，不要延迟）** — 仅在题目目录内创建 题目名称.md + exploit.py + flag.found + exp_candidate.jsonl（不写比赛级文件，由 Lead Agent 汇总）

**⚠️ 逐题交付**：每道题 `solved` 后立即完成全部题目目录内交付再进入下一题。不要等所有题做完再统一写 — 上下文压缩会丢失信息。
...
- solved 后**立即**创建 题目名称.md（如 `overflow.md`，**禁止** `wp：` 或 `wp:` 前缀）
- solved 后创建 exploit.py（Pwn/Web/Crypto 等可脚本化的题必须写，纯手工分析题可跳过）
- solved 后创建 flag.found（三行格式）：
  ```
  FLAG: flag{...}
  STATUS: solved
  TIMESTAMP: 2026-05-21T14:30:00Z
  ```
- **⚠️ 读写边界** — 可读取题目目录内全部题目文件；只写题目目录内交付文件。`limit.md`、`task_plan.md`、`findings.md`、`progress.md`、`flag.log` 等比赛级文件仅 Lead Agent 有权读写
...
Lead Agent 在 Phase 1.5 中不要手工复刻扫描/排序/Gate 规则，直接调用脚本生成调度计划；模型无需读取脚本内容：
...
只需要使用脚本 JSON 输出：
- `summary` — 总体未解/跳过/Agent 数量
- `categories[]` — 每个品类的 dispatch/skip 状态和原因
- `dispatch_agents[]` — 需要启动的 Agent 批次、题目列表、`time_budget_min`

只对 `dispatch_agents[]` 中的批次启动 Agent；若数组为空，不启动任何品类 Agent。
...
| **空返回** | Agent 返回为空或仅有错误信息 | 运行 `status.py` 扫描产物，标记剩余题目为 in_progress |
| **超时** | Agent 未在预期时间内返回 | 同空返回处理 |

**容错操作**：
1. 在 `progress.md` 记录失败的品类 Agent 及原因
2. 运行 `python3 "$SKILL_DIR/scripts/status.py" $COMPETITION_DIR --json` 查看已产出文件
3. 未完成的题目标记为 `in_progress`，在最终摘要中报告给用户
4. **不重试整个品类 Agent** — 成本高且可能重复已完成工作，由用户决定后续
...
**Lead Agent 汇总**：直接调用脚本，读取 JSON 摘要，不手工扫描：
...
| 经验库命中 | `grep -ri "关键词" $SKILL_DIR/exp/web/web.jsonl ...` (6 库全部) | 历史相似题 |
