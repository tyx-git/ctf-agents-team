...
1. 逐条校验 JSON 格式合法性
2. 去重（name+technique 组合键）后追加到对应品类的经验库文件：先逐条对比已有 .jsonl 中的 `name`+`technique` 组合，已存在则跳过
...
8. **经验库回写**：
   - **单题模式 / Lead Agent 直接解题**：追加前先检查已有 .jsonl 中的 `name`+`technique` 组合，已存在则跳过；无重复则追加到 `exp/[品类]/[品类].jsonl`
