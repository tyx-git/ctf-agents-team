...
5. **验证** — 确认 flag，达到 verified 后写最终 WP
6. **交付** — 创建 wp：题目名称.md，将 flag 写入题目目录下 flag.found
...
4. **Dispatch** — 对每个非空品类，用 Agent tool 启动一个品类 Agent
   - 所有品类 Agent **并行启动**（单条消息内多个 Agent tool call）
   - 每个 Agent 使用上方的品类 Agent Prompt 模板
5. **等待** — 所有 Agent 返回
6. **汇总** — 收集 flag.found 文件，写入根 flag.txt，更新 task_plan.md

### flag 汇总协议

避免多 Agent 并发写同一文件：
- 品类 Agent 将每道题的 flag 写入 `$COMPETITION_DIR/$CATEGORY/$CHALLENGE/flag.found`
- Lead Agent 最终扫描所有 `flag.found`，汇总到根 `flag.txt`
- 格式：`[类型][题目名称]flag字符串`
