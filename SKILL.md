...
**4.2.1 工程校验**（汇总前执行）：
```bash
python3 .skills/ctf-agents-team/scripts/val.py $COMPETITION_DIR --exp-dir .skills/ctf-agents-team/exp
```
校验目录合规、文件命名、状态一致性、经验库 schema。有错误时修复后再继续汇总。

**4.3 经验库合并**：

Lead Agent 收集所有题目目录下的 `exp_candidate.jsonl`：
1. 逐条校验 JSON 格式合法性
2. 去重后追加到对应品类的经验库文件（如 `exp/re/re.jsonl`）
...
# 搜索历史经验库
grep -ri "关键词" exp/web/web.jsonl exp/misc/misc.jsonl exp/pwn/pwn.jsonl exp/re/re.jsonl 2>/dev/null
```
...
8. **经验库回写**：
   - **单题模式 / Lead Agent 直接解题**：直接追加到 `exp/[品类]/[品类].jsonl`
   - **Solo 模式子 Agent**：写入题目目录下的 `exp_candidate.jsonl`，由 Lead Agent 最终合并
