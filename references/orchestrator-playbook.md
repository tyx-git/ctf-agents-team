...
| flag | 不直接产出 | 每题 flag 写入 `flag.found` |
| 经验库 | 不更新 | 有价值的题回写 |
...
6. **交付（立即执行，不要延迟）** — 创建 题目名称.md + exploit.py + 更新 flag.log + flag.found，有价值的经验追加到 exp/.jsonl

**⚠️ 逐题交付**：每道题 verified 后立即完成全部交付再进入下一题。不要等所有题做完再统一写 WP/更新 flag.log — 上下文压缩会丢失信息。
...
- 每 20 分钟检查收益递减
- 3-Strike Protocol: 同方向 3 次失败 → 标记 abandoned，继续下一题
- **不要在一道题上恋战** — 比赛时间有限
...
- verified 后**立即**更新比赛根目录 flag.log（不要等最后汇总）
- flag 写入题目目录下 flag.found（纯文本，一行一个 flag）
- 有价值的经验追加到 `.skills/ctf-agents-team/exp/[品类]/[品类].jsonl`（纯模板题跳过）
- 题目无法解决时：wp.process 最后 Stage 写明 blocker，标记 abandoned
...
1. **扫描** — 遍历比赛目录下所有品类子目录，收集题目列表
   ...
   - **跳过已解题目**：若题目目录内已存在 `题目名称.md`（最终 WP），直接排除
2. **分组** — 按品类分组，跳过空品类
...
2. 扫描该品类目录：`ls $CATEGORY/*/flag.found 2>/dev/null` — 检查是否有部分产出
3. 已产出 flag.found 的题目视为 solved，补充到 flag.log
...
每道题 verified 后立即更新：
- 品类 Agent 将 flag 写入 `$COMPETITION_DIR/$CATEGORY/$CHALLENGE/flag.found`
- 品类 Agent **同时立即更新**比赛根目录 `flag.log`（格式：`[类型][题目名称] flag字符串`）
- Lead Agent 最终校验 `flag.log` 完整性，扫描 `flag.found` 补充遗漏条目
