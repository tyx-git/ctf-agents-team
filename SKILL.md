...
| 类型 | 必备工具 |
|------|---------|
| Pwn | `checksec`, `gdb`/`gdb-multiarch`, `patchelf`, Python `pwntools` |
| Web | `curl`, `jq`, `sqlmap`(可选), `ffuf`/`gobuster`(可选) |
| Reverse | `radare2`, `binwalk`, `strings`, `objdump` |
| Mobile | `apktool`, `jadx`, `adb`, Java runtime |
| Crypto | Python `pycryptodome`, `sympy`, `z3-solver`, `sage`(可选) |
| Misc | `ffmpeg`, `exiftool`, `tshark`, `zbar-tools`, `steghide` |
| Forensics | `volatility3`, `foremost`, `tshark`, `binwalk` |

缺失工具优先用 pip 安装，apt 包提示用户手动安装：
```bash
pip3 install pwntools pycryptodome z3-solver sympy
# apt 包需要 sudo，提示用户：
# 请执行: sudo apt install -y radare2 binwalk steghide tshark
```

工具完整列表和路径见 `workspace.json`。
详见 [references/environment-baseline.md](references/environment-baseline.md)。
...
CTF 比赛有严格时间限制。合理分配时间是取得好成绩的关键。

**题目选择优先级评分**（Phase 1 入场后对所有题目评分）：
...
- 如果连续两个检查点无新发现 → 触发 Pivot 或 Abandoned
